...
func testParseFile(filename string) error {
	...
	sep := "// ==========================="
	code := strings.SplitN(string(file), sep, 3)
	p := NewParser(scanner.NewStringScan(filename, code[0], nil), errHandler)
	t := p.ParseUnit()

	dump := ast.DumpString(t, "// ", " ")
	if len(code) == 1 {
		return ioutil.WriteFile(filename, []byte(fmt.Sprintf("%s\n%s\n%s%s\n", code[0], sep, dump, sep)), os.ModePerm)
	}
	code[1] = strings.ReplaceAll(code[1], "\r\n", "\n")
	if strings.TrimSpace(dump) != strings.TrimSpace(code[1]) {
		return stderr.New(fmt.Sprintf("parse error: code:\n%s\nwant:\n%s\ngot:\n%s\n", code[0], code[1], dump))
	}
	return nil
}
