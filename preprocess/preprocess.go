	...
	if v, ok := p.ctx.Val[name]; ok {
		...
		case *MacroFunc:
			...
			if t, tks, ok := p.expandFunc(tok, val); ok {
				d := calcDelta(t, tks)
				// fmt.Println("expand", d, token.String(tok), inlineTokenString(t), len(inlineTokenString(t)), "=>", inlineTokenString(tks), len(inlineTokenString(tks)))
				p.deltaLine(d)
				p.push(tks)
				p.next()
				return true
...
func (p *processor) expandFunc(tok token.Token, val *MacroFunc) ([]token.Token, []token.Token, bool) {
	...
	total := c.GetClear()
	total = append([]token.Token{tok}, total[:len(total)-1]...)
	// fmt.Println(token.String(tok), "param", printTokens(total))
	body := p.expandMacroBody(tok, val.Body, params)
	// fmt.Println(token.String(tok), printTokens(val.Body), "=>", printTokens(body))
...
func (p *processor) expandMacroBodyToken(tok, ident token.Token, params map[string][]token.Token, afterHashHash, followHashHash bool) (exp []token.Token) {
	...
	if ident.Type() != token.IDENT {
		exp = []token.Token{ident}
		calcExpandPos(ident, exp)
		return
	}
...
func calcExpandPos(tok token.Token, tks []token.Token) {
	pos := tok.Position()
	base := tks[0].Position()
	for i := range tks {
...
func makeExpand(tok token.Token, tks []token.Token) {
	for i := range tks {
		tks[i] = &Token{
			Pos:    tks[i].Position(),
			Typ:    tks[i].Type(),
