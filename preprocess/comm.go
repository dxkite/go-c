...
func tokenString(tks []token.Token) string {
	...
	for _, tok := range tks {
		...
		col += utf8.RuneCountInString(tok.Literal())
		str += tok.Literal()
	}
	return str
}
...
func printTokens(tks []token.Token) string {
	var ele []string
	for _, t := range tks {
		ele = append(ele, token.String(t))
	}
	return fmt.Sprintf("%s {%s}", strconv.QuoteToGraphic(inlineTokenString(tks)), strings.Join(ele, " "))
}
